document.addEventListener("DOMContentLoaded", () => {
    const root = document.querySelector(".genres-page");
    if (!root) return;

    console.log("genres.js activo");
    ...
    const form = root.querySelector(".js-genre-form");
    ...
    const resetBtn = form.querySelector(".js-genre-reset");
    const errorsEl = form.querySelector(".js-errors");

    const genreIdEl = form.querySelector("#genre_id");
    ...
    async function loadGenre(id, tabEl) {
        setErrors([]);
        const url = `${baseUrl}/${id}`;

        const res = await fetch(url, { headers: headersJson() });
        if (!res.ok) {
            setErrors([`No se pudo cargar el género (HTTP ${res.status}).`]);
            return;
        }

        const data = await res.json();

        genreIdEl.value = data.id;
    ...
    async function saveGenre() {
        ...
        await refreshList();
        await loadGenre(json.id);
    }

    async function deleteGenre() {
        const id = genreIdEl.value;
        if (!id) return;
    ...
    function bindListClicks() {
        root.querySelectorAll(".edit-tab").forEach(tab => {
            tab.addEventListener("click", () => loadGenre(tab.dataset.id, tab));
        });
    }
    ...
    deleteBtn?.addEventListener("click", deleteGenre);
