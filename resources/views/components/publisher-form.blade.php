<form class="js-publisher-form" method="POST" action="{{ route('admin.publishers.store') }}"
    data-store-url="{{ route('admin.publishers.store') }}" data-show-url-base="{{ url('/admin/publishers') }}">
    @csrf
    ...
    <div class="buttons">
        ...
        <button type="button" class="js-publisher-reset" title="Limpiar">
            <x-icon.broom />
        </button>
    ...
    <div class="form-fields">
        <div>
            ...
            <input id="name" name="name" type="text" placeholder="Penguin Random House">
        </div>
